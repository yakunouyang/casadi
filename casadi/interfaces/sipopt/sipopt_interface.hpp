...
#include "casadi_nlpsol_sipopt_export.h"
...
namespace casadi {
  ...
  struct CASADI_NLPSOL_SIPOPT_EXPORT SIpoptMemory : public NlpsolMemory {
    /** NOTE:
...
*/
  class CASADI_NLPSOL_SIPOPT_EXPORT SIpoptInterface : public Nlpsol {
    friend class SIpoptUserClass;

  public:
    Sparsity jacg_sp_, jacgp_sp_;
    Sparsity hesslag_sp_, hesslagp_sp_;

    explicit SIpoptInterface(const std::string &name, const Function &nlp);

    ~SIpoptInterface() override;

// Get name of the plugin
    const char *plugin_name() const override { return "sipopt"; }

// Get name of the class
    std::string class_name() const override { return "SSIpoptInterface"; }

/** \brief  Create a new NLP Solver */
    static Nlpsol *creator(const std::string &name, const Function &nlp) {
      return new SIpoptInterface(name, nlp);
    }
...
/** \brief Create memory block */
    void *alloc_mem() const override { return new SIpoptMemory(); }

/** \brief Initalize memory block */
    int init_mem(void *mem) const override;

/** \brief Free memory block */
    void free_mem(void *mem) const override { delete static_cast<SIpoptMemory *>(mem); }
...
// Ipopt callback functions
    void finalize_solution(SIpoptMemory *m, const double *x, const double *z_L, const double *z_U,
                           const double *g, const double *lambda, double obj_value,
                           int iter_count, const std::vector<double> &perturbed_x) const;

    bool get_bounds_info(SIpoptMemory *m, double *x_l, double *x_u,
                         double *g_l, double *g_u) const;

    bool get_starting_point(SIpoptMemory *m, bool init_x, double *x,
                            bool init_z, double *z_L, double *z_U,
                            bool init_lambda, double *lambda) const;

    void get_nlp_info(SIpoptMemory *m, int &nx, int &ng,
    ...
    bool intermediate_callback(SIpoptMemory *m, const double *x, const double *z_L,
    ...
    bool get_var_con_metadata(SIpoptMemory *m, std::map<std::string, std::vector<std::string> > &var_string_md,
                              std::map<std::string, std::vector<int> > &var_integer_md,
                              std::map<std::string, std::vector<double> > &var_numeric_md,
                              std::map<std::string, std::vector<std::string> > &con_string_md,
                              std::map<std::string, std::vector<int> > &con_integer_md,
...
// Options
    ...
    std::vector<double> perturbed_p_;
...
/** \brief Deserialize into MX */
    static ProtoFunction *deserialize(DeserializingStream &s) { return new SIpoptInterface(s); }

  protected:
/** \brief Deserializing constructor */
    explicit SIpoptInterface(DeserializingStream &s);
  };
