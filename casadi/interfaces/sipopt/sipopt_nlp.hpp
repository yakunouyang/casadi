...
namespace casadi {
  ...
  class SIpoptInterface;
  struct SIpoptMemory;

  class CASADI_NLPSOL_SIPOPT_EXPORT SIpoptUserClass : public TNLP {
#ifdef WITH_IPOPT_CALLBACK
  friend class TNLPAdapter;
#endif // WITH_IPOPT_CALLBACK

  public:
  SIpoptUserClass(const SIpoptInterface& solver, SIpoptMemory* mem);
  ~SIpoptUserClass() override;
  ...
  private:
  SIpoptUserClass(const SIpoptUserClass&);
  SIpoptUserClass& operator=(const SIpoptUserClass&);
  const SIpoptInterface& solver_;
  SIpoptMemory* mem_;
